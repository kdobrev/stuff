...
LOOP AT lt_fields ASSIGNING FIELD-SYMBOL(<fs_field>).
  ...
  IF sy-subrc = 0.
    <fs_total> = REDUCE i(
      ...
      NEXT total = total + VALUE i(
        LET field_val = VALUE #( 
          ASSIGN COMPONENT <fs_field>-name OF STRUCTURE <fs_line> TO <fs_total>.
          IF sy-subrc = 0.
            <fs_total>.
          ELSE.
            0.
